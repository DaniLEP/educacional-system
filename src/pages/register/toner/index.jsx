...
export default function CadastroToner() {
  ...
  async function handleSubmit(e) {
    ...
    if (!sku || quantidade < 1) {setNotif({ essage: "SKU e quantidade válidos são obrigatórios.", tipo: "error",});
      return;
    }
    try {
      ...
      else {await push(ref(db, "toners"), { cor, sku, impressora, quantidade });
      ...
      <motion.div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-600 to-gray-900 p-6" initial={{ opacity: 0 }}animate={{ opacity: 1 }}>
        <motion.div className="bg-white rounded-2xl shadow-2xl p-10 w-full max-w-4xl" initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.4 }}>
          <h2 className="text-3xl md:text-4xl font-extrabold text-gray-800 text-center mb-10 flex items-center justify-center gap-3">
            <Printer className="text-indigo-600 w-8 h-8" />Cadastro de Toner </h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <Input required type="text" name="sku" value={formData.sku} onChange={handleChange} placeholder="Digite o SKU"
                className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition" />
            </div>
            {/* Impressora */}
            <div>
            ...
            <div>
              ...
              <Input required type="number" name="quantidade" min={1} value={formData.quantidade} onChange={handleChange}
                className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition" />
            </div>
            {/* Botões */}
