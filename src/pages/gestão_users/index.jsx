 ...
 useEffect(() => {
  ...
  const unsubscribe = onAuthStateChanged(auth, (user) => {
    ...
    onValue(connectedRef, (snapshot) => {
      ...
      // Define o que acontece ao desconectar (atualizar apenas os campos de presença)
      onDisconnect(statusRef)
        ...
        .then(() => {
          // Ao conectar, atualize apenas os campos online e lastSeen
          update(statusRef, {
            online: true,
            lastSeen: serverTimestamp(),
          });
  ...
  // Filtrar usuários por busca e status
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const notifRef = ref(db, `notificacoes/${userId}`)
  ...
  function handleClickNotificacoes() {
    navigate("/painel/notificacoes") // ajuste para sua rota de notificações
  }
    ...
    <div className="min-h-screen bg-gray-50/50 p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          {/* Card de Notificações Funcional */}
          <Card
            onClick={handleClickNotificacoes}
            className="bg-gradient-to-r from-green-500 to-green-600 text-white cursor-pointer hover:brightness-110 transition"
          >
            <CardContent className="p-6 flex items-center justify-between">
              <div>
                <p className="text-green-100 text-sm font-medium">Notificações</p>
                <p className="text-3xl font-bold">
                  {notificacoes.length > 0 ? `${notificacoes.length} nova${notificacoes.length > 1 ? "s" : ""}` : "Nenhuma nova"}
                </p>
              </div>
              <div className="relative">
                <Bell className="w-8 h-8 text-green-200" />
                {notificacoes.length > 0 && (
                  <span className="absolute -top-1 -right-1 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 bg-red-600 rounded-full animate-pulse">
                    {notificacoes.length}
                  </span>
                )}
              </div>
